...
db.serialize(() => {
    ...
    db.get("SELECT COUNT(*) as count FROM admin_keys", (err, row) => {
        if (row.count === 0) {
            db.run("INSERT INTO admin_keys VALUES (?, ?, ?, ?, ?)", 
                ['admin', 'Super Admin', 'superadmin', 'admin2024', JSON.stringify(['users','reports','gifts','messages'])]);
        }
    });
    
    // Добавляем дефолтные подарки
...
app.get('/api/users', (req, res) => {
    db.all("SELECT id, name, username, avatar, stars, tags, banned, muted, bio FROM users", (err, users) => {
        ...
        users.forEach(u => {
            if (u.tags) {
                try { u.tags = JSON.parse(u.tags); } catch(e) { u.tags = []; }
            } else { u.tags = []; }
        });
        res.json(users);
    });
...
app.post('/api/gifts', (req, res) => {
    const { name, emoji, type, price, stock } = req.body;
    const giftId = 'g' + Date.now();
    
    db.run("INSERT INTO gifts VALUES (?, ?, ?, ?, ?, ?)",
        [giftId, name, emoji, type, price, stock],
        (err) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true });
        }
...
app.delete('/api/gifts/:id', (req, res) => {
    const { id } = req.params;
    db.run("DELETE FROM gifts WHERE id = ?", [id], (err) => {
        res.json({ success: true });
    });
});
...
app.post('/api/admin/login', (req, res) => {
    ...
    db.get("SELECT * FROM admin_keys WHERE (id = ? OR name = ?)", [login, login], (err, admin) => {
        if (!admin) return res.status(401).json({ error: 'Неверный логин' });
        
        if (admin.pass !== password) return res.status(401).json({ error: 'Неверный пароль' });
        
        let perms = [];
        try { perms = JSON.parse(admin.perms); } catch(e) {}
