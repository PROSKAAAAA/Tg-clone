...
app.get('/api/messages/:userId', (req, res) => {
    ...
    const msgs = data.messages[dialogId] || [];
    // Обновляем last_seen получателя
    const receiver = data.users[req.params.userId];
    if (receiver) {
...
// ============ БОТ SPAM INFO (упрощённый) ============
const botAppeals = [];

app.get('/api/bot/appeals', (req, res) => {
    res.json(botAppeals.filter(a => a.status === 'pending'));
});

app.post('/api/bot/appeals/:id', (req, res) => {
    const appeal = botAppeals.find(a => a.id === req.params.id);
    if (appeal) {
        appeal.status = req.body.status;
        appeal.adminComment = req.body.adminComment;
        if (req.body.status === 'approved') {
            const user = data.users[appeal.userId];
            if (user) {
                user.spamBlocked = false;
                user.spamReason = null;
                user.spamUntil = null;
                saveData();
                sendNotification(appeal.userId, '✅ Апелляция одобрена', `Ваш спам-блок снят!`);
            }
        }
        saveData();
    }
